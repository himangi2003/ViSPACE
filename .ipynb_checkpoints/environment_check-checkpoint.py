...
=====================
Pre-flight environment check for the ViP-SegD pipeline.

Run this BEFORE `run_vipsegd.py` (or any individual stage script) to catch
...
6. Config paths: WSI_PATH, CHECKPOINT, MUSSEL_DIR all exist on disk
...
def check_python_version(min_version=(3, 10)) -> bool:
    ...
    print(f"{_FAIL}Python {v.major}.{v.minor} is older than the recommended "
          f"{min_version[0]}.{min_version[1]} (ViP-SegD_environment.yml pins 3.11)")
    return False
...
def check_required_packages() -> bool:
    ...
    for import_name, pip_name in REQUIRED_PACKAGES:
        try:
            ...
            all_ok = False
            print(f"{_FAIL}{import_name:<18} ({pip_name}) — NOT INSTALLED")
...
def check_gpu(device_pref: str = "cuda") -> bool:
    ...
    try:
        ...
        if "busy or unavailable" in err:
            ...
            print(f"           Try: CUDA_LAUNCH_BLOCKING=1 python run_vipsegd.py ...")
        return False
...
def check_huggingface_auth(virchow2_path: Optional[str] = None) -> bool:
    ...
    if virchow2_path:
        ...
        if p.exists():
            ...
            print(f"           HF login not required — loading from local weights.")
            return True
        else:
...
def check_config_paths(cfg) -> bool:
    ...
    checks = [
        ...
        ("CHECKPOINT", cfg.CHECKPOINT, "the ViP-SegD model checkpoint (.pt)"),
    ]
    ...
    if out_dir.exists():
        print(f"{_OK}cfg.OUT_DIR     exists: {cfg.OUT_DIR}")
    else:
        print(f"{_WARN}cfg.OUT_DIR does not exist yet — will be created on first run.")
...
def run_environment_check(cfg=None) -> bool:
    ...
    print()
    if all_passed:
        print("  All required checks passed — safe to run the pipeline.")
    else:
        print("  One or more required checks FAILED — fix the items above "
